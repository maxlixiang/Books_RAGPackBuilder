...
早期最直接的方案是按固定字数分块，例如每 700 字左右切成一个 chunk，超过 1100 字就强制切开，并在相邻 chunk 之间保留少量 overlap。这种方案实现简单，也适合没有结构的长文本，但用于书籍时有明显问题：

```text
1. 容易把一个完整观点从中间切断
2. 一个 chunk 可能同时混入两个不同小节的内容
3. 作者原本设计好的章节、小节、论证顺序没有被充分利用
4. 向量表示会变“杂”，检索命中后给大模型的上下文也不够完整
```

因此，本项目改成“先按结构，后按长度”的分块方式。程序会优先把一本书中天然完整的小节保留下来，只有当小节本身过长、已经包含多个子观点或多个案例时，才按段落 / 句子做二次切分。

这是本项目的核心亮点之一：它不是把书当成一条没有结构的长文本，而是把书当成作者已经组织好的知识结构来处理。
...
有些 PDF 的左侧书签 / 目录层级比较浅，只列到“章”，但正文内部其实还有很多清晰的短标题。比如《如何阅读一本书》的目录可能只列到“第六章 一本书的分类”，但正文中还有“书籍分类的重要性”“从一本书的书名中你能学到什么”“实用性 vs. 理论性作品”等短标题。

如果只依赖 PDF 书签，这一整章会先被当成一个很大的自然小节，然后再因为过长被按段落 / 字数拆开。这样虽然能避免超长 chunk，但切分边界不一定对应真实语义主题。

正文短标题增强就是为了解决这个问题：当 PDF 书签层级偏浅时，程序会扫描正文中疑似独立标题的短行，把这些标题补充成更细的结构边界，让 chunk 更接近作者在正文里实际划分的主题。

这个功能的价值是：

```text
1. 弥补 PDF 书签不完整的问题
2. 避免只按章级目录导致 chunk 过大
3. 比固定字数二次切分更接近真实语义边界
4. 提高后续向量检索时的主题纯度
5. 让引用路径更具体，例如“书名 > 第六章 > 书籍分类的重要性”
```
